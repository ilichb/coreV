  ...
  return (
    <div className="px-4 md:px-6 lg:px-8 py-6 md:py-8">
      ...
      <div className="flex items-center justify-between mb-10 border-b border-reactor-cyan/10 pb-6">
        <div>
          <h2 className="text-base md:text-xl font-mono-display font-bold text-gray-100 tracking-[0.15em] md:tracking-[0.2em] mb-1 uppercase break-words">
            {tScorecard('title')}
          </h2>
          <p className="text-[8px] md:text-[8px] md:text-[10px] text-gray-500 font-mono-display uppercase tracking-wider leading-tight break-words">
            {tScorecard('subtitle')}
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 bg-purple-500/10 border border-purple-500/40 text-[8px] md:text-[10px] font-mono-display font-bold text-gray-300 hover:text-purple-400 hover:bg-purple-500/20 transition-all uppercase tracking-widest rounded-[2px]"
          >
            {tActions('testChallenge')}
          </button>
          <button
            onClick={handleValidate}
            className="px-4 py-2 bg-reactor-cyan/10 border border-reactor-cyan/40 text-[8px] md:text-[10px] font-mono-display font-bold text-gray-300 hover:text-reactor-cyan hover:bg-reactor-cyan/20 transition-all uppercase tracking-widest rounded-[2px]"
          >
      ...
      {apiTest && (
        <div className="mb-8 p-4 bg-black/60 border border-reactor-cyan/20 rounded-[2px] animate-in slide-in-from-top duration-300">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-[8px] md:text-[10px] font-mono-display font-bold text-reactor-cyan uppercase tracking-widest">
              API_GATEWAY_RESPONSE_STAMP // {apiTest.type.toUpperCase()}
            </h4>
          </div>
          <pre className="text-[8px] md:text-[10px] text-gray-400 font-mono-display bg-black/40 p-3 overflow-auto max-h-40 custom-scrollbar border border-white/5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-12">
        {steps.map((s) => (
          ...
          >
            ...
            <span className="text-[8px] md:text-[10px] font-mono-display font-bold uppercase tracking-widest">{s.label}</span>
            {step === s.id && (
      ...
      {/* Form Content */}
      <div className="min-h-[250px] md:min-h-[300px] mb-12 bg-black/20 border border-white/5 p-6 rounded-[2px] relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label className="text-[8px] md:text-[10px] text-gray-500 font-mono-display uppercase tracking-widest">{t('sections.minDuration')}</label>
                <input
                  type="number"
              ...
              <div className="space-y-3">
                <label className="text-[8px] md:text-[10px] text-gray-500 font-mono-display uppercase tracking-widest">{t('sections.maxDuration')}</label>
                <input
                  type="number"
                  className="w-full bg-black/40 border border-white/10 rounded-[2px] p-3 text-sm font-mono-display text-reactor-cyan outline-none focus:border-reactor-cyan/50 tabular-nums"
        ...
        {step === 5 && (
          <div className="relative z-10 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-6 pt-8 border-t border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="text-[8px] md:text-[10px] text-gray-400 font-mono-display uppercase tracking-widest font-bold">{t('sections.telegramLabel')}</label>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="text-[8px] md:text-[10px] text-gray-400 font-mono-display uppercase tracking-widest font-bold">{t('sections.emailLabel')}</label>
              ...
              <div className="p-3 md:p-4 bg-reactor-cyan/5 border border-reactor-cyan/10 rounded-[2px]">
      ...
      {/* Navigation Controls */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8 border-t border-reactor-cyan/10">
        <button
          onClick={handlePrev}
          disabled={step === 1}
          className="group flex items-center gap-3 px-4 py-2 md:px-6 md:py-3 border border-gray-600 bg-white/5 rounded-[2px] font-mono-display text-[8px] md:text-[10px] font-bold text-gray-100 hover:text-reactor-cyan hover:border-reactor-cyan/50 hover:bg-reactor-cyan/5 transition-all uppercase tracking-[0.2em] disabled:opacity-20 disabled:cursor-not-allowed"
        >
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <button
            onClick={handleValidate}
            className="px-4 py-2 md:px-8 md:py-3 bg-black/60 border border-reactor-cyan/50 text-gray-100 font-mono-display text-[8px] md:text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-reactor-cyan/10 hover:text-reactor-cyan hover:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all rounded-[2px]"
          >
          ...
          {step < 6 ? (
            <button
              onClick={handleNext}
              className="group flex items-center gap-3 px-4 py-2 md:px-10 md:py-3 bg-black/80 border border-reactor-cyan text-gray-100 font-mono-display text-xs font-bold uppercase tracking-[0.2em] hover:bg-reactor-cyan hover:text-black hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] active:scale-[0.98] transition-all rounded-[2px]"
            >
              {t('navigation.next')}
            ...
            <div className="flex items-center justify-between mb-8 border-b border-green-500/10 pb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[8px] md:text-[10px] text-green-500/60 font-mono-display uppercase tracking-widest mt-1">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="bg-black/60 border border-white/5 p-4 rounded-[1px]">
                    <p className="text-[8px] md:text-[10px] font-mono text-green-500/80 break-all leading-relaxed">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white/5 p-4 border border-white/5 rounded-[1px] flex items-center justify-between group cursor-pointer hover:bg-white/10 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[8px] md:text-[10px] font-mono-display text-gray-400 uppercase tracking-widest">{t('proof.viewIpfs')}</span>
                  </div>
            ...
            <div className="flex-1">
              ...
              {validation.errors.length > 0 && (
                <div className="bg-black/40 p-4 border border-white/5 space-y-2">
                  <p className="text-[8px] md:text-[10px] font-mono-display font-bold text-red-400 uppercase tracking-widest mb-2">{t('report.errors')}[{validation.errors.length}]</p>
                  {validation.errors.map((error, index) => (
                    <div key={index} className="flex gap-2 text-[8px] md:text-[10px] font-mono-display text-gray-400 uppercase">
