...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Ejecutar workflow de Atlas para convertir el Scorecard en milestone y pagar recompensas
      try {
        const atlasResult = await atlasOrchestrator.executeFullWorkflow(
          scorecard as SignedScorecard,
          ipfsResult.cid,
          validationResult.clarityDelta
        );
        if (atlasResult.success) {
          logger.info(`✅ Atlas workflow completed: ${atlasResult.finalStatus}`);
        } else {
          logger.warn(`⚠️ Atlas workflow issues: ${atlasResult.errors.join(", ")}`);
        }
      } catch (atlasErr) {
        logger.error("❌ Atlas workflow failed (non-critical):", atlasErr);
      }
